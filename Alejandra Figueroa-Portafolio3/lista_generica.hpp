...
bool Lista<TIPODATO>::estaVacia() {
	cout << "Comprovando si la lista esta vacia..." << endl;
...
void Lista<TIPODATO>::insertar(int indice, TIPODATO item) {
	...
	// Desplazar elementos a partir del indice a laderecha
	...
	cout << "Insertando elemneto " << item << " en el indice " << indice << endl;
	this->items[indice] = item;
	
	// Incrmentar la cuenta de elementos
...
TIPODATO Lista<TIPODATO>::obtener(int indice) {
	// Validar el indice
	if (indice < 0 || indice >= this->cuenta)  throw "Indice fuera de rango";
	if (this->estaVacia()) throw "No se peude recuperar elementos de una lista vacia";
...
TIPODATO Lista<TIPODATO>::remover(int indice) {
	...
	cout << "removiendo el elemnto con insice " << indice << "..." << endl;
